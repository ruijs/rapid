...
- **{RockName}Props**: 继承 `RockInstance`，包含组件所需的业务属性（如 value、onChange 等）
...
export interface RapidDatePickerProps extends RockInstance {
...
import { RockInstance } from "@ruiapp/move-style";
import { RapidNumberRendererRockConfig } from "../rapid-number-renderer/rapid-number-renderer-types";

export interface RapidCurrencyRendererProps extends RockInstance {
...
export interface RapidDictionaryEntryRendererProps extends RockInstance {
...
2. **类型继承**:
   ...
   - {RockName}Props 继承 `RockInstance`，以便在组件中访问 `$id`、`$type` 等 Rock 实例属性
